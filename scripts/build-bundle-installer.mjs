...
function createNsisScript({ outputExePath, payloadDirPath, appVersion }) {
  ...
  const lines = [
    ...
    'SectionEnd',
    '',
    'Section /o "F95 Module" SEC_F95_MODULE',
    'SectionEnd',
    '',
    ...
    '  ${EndIf}',
    '  ${If} ${SectionIsSelected} ${SEC_SERVICE}',
    '    ${If} ${SectionIsSelected} ${SEC_F95_MODULE}',
    '      Call EnableF95ModuleForService',
    '    ${Else}',
    '      Call DisableF95ModuleForService',
    '    ${EndIf}',
    '  ${ElseIf} ${SectionIsSelected} ${SEC_WEB}',
    '    ${If} ${SectionIsSelected} ${SEC_F95_MODULE}',
    '      Call EnableF95ModuleForService',
    '    ${Else}',
    '      Call DisableF95ModuleForService',
    ...
    '    ${If} ${SectionIsSelected} ${SEC_F95_MODULE}',
    '      Call EnableF95ModuleForStandaloneClient',
    '    ${Else}',
    '      Call DisableF95ModuleForStandaloneClient',
    '    ${EndIf}',
    '  ${EndIf}',
    '  ${If} ${SectionIsSelected} ${SEC_WEB}',
    '    ${Unless} ${SectionIsSelected} ${SEC_F95_MODULE}',
    '      Push "$INSTDIR\\dist-web-client\\modules"',
    '      Call RemoveF95ModulePayload',
    '    ${EndUnless}',
    '  ${EndIf}',
    '  ${If} ${SectionIsSelected} ${SEC_DESKTOP}',
    '    ${Unless} ${SectionIsSelected} ${SEC_F95_MODULE}',
    '      Push "$INSTDIR\\dist-standalone-client\\modules"',
    '      Call RemoveF95ModulePayload',
    '    ${EndUnless}',
    '  ${EndIf}',
    ...
    'FunctionEnd',
    '',
    'Function EnableF95ModuleForService',
    '  Push "$APPDATA\\Local Game Gallery Service Tray\\config.json"',
    '  Push "true"',
    '  Call SeedF95ModuleConfig',
    'FunctionEnd',
    '',
    'Function DisableF95ModuleForService',
    '  Push "$APPDATA\\Local Game Gallery Service Tray\\config.json"',
    '  Push "false"',
    '  Call SeedF95ModuleConfig',
    'FunctionEnd',
    '',
    'Function EnableF95ModuleForStandaloneClient',
    '  Push "$APPDATA\\Local Game Gallery Client\\config.json"',
    '  Push "true"',
    '  Call SeedF95ModuleConfig',
    'FunctionEnd',
    '',
    'Function DisableF95ModuleForStandaloneClient',
    '  Push "$APPDATA\\Local Game Gallery Client\\config.json"',
    '  Push "false"',
    '  Call SeedF95ModuleConfig',
    'FunctionEnd',
    '',
    'Function SeedF95ModuleConfig',
    '  Pop $0',
    '  Pop $1',
    '  Push $2',
    '  Push $3',
    ...
    '  ${GetParent} "$1" $2',
    '  CreateDirectory "$2"',
    '  StrCpy $3 "$TEMP\\lgg-configure-f95.ps1"',
    '  FileOpen $4 "$3" w',
    '  FileWriteUTF16LE $4 "$$configPath = $$args[0]$\\r$\\n"',
    '  FileWriteUTF16LE $4 "$$enabled = [System.Convert]::ToBoolean($$args[1])$\\r$\\n"',
    '  FileWriteUTF16LE $4 "$$config = [pscustomobject]@{}$\\r$\\n"',
    '  FileWriteUTF16LE $4 "if (Test-Path $$configPath) { try { $$loaded = Get-Content -Raw -Path $$configPath | ConvertFrom-Json; if ($$loaded) { $$config = $$loaded } } catch { $$config = [pscustomobject]@{} } }$\\r$\\n"',
    '  FileWriteUTF16LE $4 "if (-not $$config.modules) { $$config | Add-Member -NotePropertyName modules -NotePropertyValue ([pscustomobject]@{}) -Force }$\\r$\\n"',
    '  FileWriteUTF16LE $4 "if (-not $$config.modules.f95) { $$config.modules | Add-Member -NotePropertyName f95 -NotePropertyValue ([pscustomobject]@{ installed = $$enabled; enabled = $$enabled; state = [pscustomobject]@{} }) -Force }$\\r$\\n"',
    '  FileWriteUTF16LE $4 "$$config.modules.f95 | Add-Member -NotePropertyName installed -NotePropertyValue $$enabled -Force$\\r$\\n"',
    '  FileWriteUTF16LE $4 "if (-not $$config.modules.f95.state) { $$config.modules.f95 | Add-Member -NotePropertyName state -NotePropertyValue ([pscustomobject]@{}) -Force }$\\r$\\n"',
    '  FileWriteUTF16LE $4 "$$config.modules.f95.installed = $$enabled$\\r$\\n"',
    '  FileWriteUTF16LE $4 "$$config.modules.f95.enabled = $$enabled$\\r$\\n"',
    '  FileWriteUTF16LE $4 "$$config | ConvertTo-Json -Depth 100 | Set-Content -Path $$configPath -Encoding UTF8$\\r$\\n"',
    '  FileClose $4',
    '  nsExec::ExecToLog \'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$3" "$1" "$0"\'',
    '  Delete "$3"',
    '  Pop $4',
    '  Pop $3',
    '  Pop $2',
    'FunctionEnd',
    '',
    'Function RemoveF95ModulePayload',
    '  Exch $0',
    '  Push $1',
    '  CreateDirectory "$0"',
    '  RMDir /r "$0\\f95"',
    '  StrCpy $1 "$0\\module-index.js"',
    '  FileOpen $1 "$1" w',
    '  FileWrite $1 "const installedModuleManifests = [];$\\r$\\n"',
    '  FileWrite $1 "export { installedModuleManifests };$\\r$\\n"',
    '  FileWrite $1 "export default installedModuleManifests;$\\r$\\n"',
    '  FileClose $1',
    '  Pop $1',
    '  Pop $0',
    'FunctionEnd',
